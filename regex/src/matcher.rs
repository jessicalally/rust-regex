...
fn match_atom(atom : &Atom, s : &str) -> Option<(String, String)> {
    match atom {
        AtomExpr(expr) => match_expression(expr.clone().to_vec(), s),
        AtomCh(c) => match_character(*c, s),
        CharClass(members) => match_character_class(members.clone().to_vec(), s),
    }
}
...
fn match_expression(expr : Expr, s : &str) -> Option<(String, String)> {   
    expr.iter()
        .fold(Some((String::new(), s.to_string())), |acc, x| {
            ...
            let (next_matched, rest) = match_term(x.clone(), &rest)?;
            Some((format!("{}{}", matched, next_matched), rest))
        })
}

pub fn matcher(expr : Expr, s : &str) -> Option<String> {
